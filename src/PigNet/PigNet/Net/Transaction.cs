...
public class ItemStackRequests : List<ItemStackActionList>, IPacketDataObject
{
	public void Write(Packet packet)
	{
		...
		foreach (ItemStackActionList request in this) packet.Write(request);
	}
	...
	public static ItemStackRequests Read(Packet packet)
	{
		...
		for (int i = 0; i < count; i++) request.Add(ItemStackActionList.Read(packet));
		return request;
	}
}
...
public class ItemStackActionList : List<ItemStackAction>, IPacketDataObject
{
	...
	public void Write(Packet packet)
	{
		...
		packet.WriteLength(Count);
		foreach (ItemStackAction action in this) packet.Write(action);
		packet.WriteLength(FilterStrings.Count);
		foreach (string filter in FilterStrings) packet.Write(filter);
	...
	{
		var actions = new ItemStackActionList { RequestId = packet.ReadVarInt() };

		int actionsCount = packet.ReadLength();
		for (int i = 0; i < actionsCount; i++) actions.Add(ItemStackAction.Read(packet));
		int filtersCount = packet.ReadLength();
		for (int i = 0; i < filtersCount; i++) actions.FilterStrings.Add(packet.ReadString());
...
public class FullContainerName : IPacketDataObject
{
	...
	public void Write(Packet packet)
	{
		...
		if (DynamicId.HasValue) packet.Write(DynamicId.Value);
	}
...
public class TakeAction : TakeOrPlaceAction
{
	...
	{
		return ReadData(packet, new TakeAction());
	}
}
...
public class PlaceAction : TakeOrPlaceAction
{
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		return ReadData(packet, new PlaceAction());
	}
}
...
public class DestroyAction : DisappearStackRequestAction
{
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		return ReadData(packet, new DestroyAction());
	}
}
...
public class ConsumeAction : DisappearStackRequestAction
{
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		return ReadData(packet, new ConsumeAction());
	}
}
...
public class CreateAction : ItemStackAction
{
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		return new CreateAction { ResultSlot = packet.ReadByte() };
	}
}
...
public class CraftAutoAction : ItemStackAction
{
	...
	protected override void WriteData(Packet packet)
	{
		...
		foreach (RecipeIngredient ingredient in RecipeIngredients) packet.Write(ingredient);
	}
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		...
		for (int i = 0; i < count; i++) action.RecipeIngredients.Add(RecipeIngredient.Read(packet));
		return action;
	}
}
...
public class LoomStackRequestAction : ItemStackAction
{
	...
	internal static ItemStackAction ReadData(Packet packet)
	{
		return new LoomStackRequestAction { PatternId = packet.ReadString() };
	}
}
...
public class ItemStackResponses : List<ItemStackResponse>, IPacketDataObject
{
	...
	{
		packet.WriteLength(Count);
		foreach (ItemStackResponse response in this) packet.Write(response);
	}
	...
	public static ItemStackResponses Read(Packet packet)
	{
		...
		int count = packet.ReadLength();
		for (int i = 0; i < count; i++) responses.Add(ItemStackResponse.Read(packet));
		return responses;
	}
}
...
public class ItemStackResponse : IPacketDataObject
{
	...
	public List<StackResponseContainerInfo> ResponseContainerInfos { get; set; } = [];
	...
	public void Write(Packet packet)
	{
		...
		foreach (StackResponseContainerInfo response in ResponseContainerInfos) packet.Write(response);
	}
	...
	public static ItemStackResponse Read(Packet packet)
	{
		...
		int count = packet.ReadLength();
		for (int i = 0; i < count; i++) response.ResponseContainerInfos.Add(StackResponseContainerInfo.Read(packet));
...
public class StackResponseContainerInfo : IPacketDataObject
{
	...
	public void Write(Packet packet)
	{
		...
		foreach (StackResponseSlotInfo slot in Slots) packet.Write(slot);
	}

	public static StackResponseContainerInfo Read(Packet packet)
	{
		var response = new StackResponseContainerInfo { ContainerName = FullContainerName.Read(packet) };
		int count = packet.ReadLength();
		for (int i = 0; i < count; i++) response.Slots.Add(StackResponseSlotInfo.Read(packet));
		return response;
	}
}
...
public abstract class Transaction : IPacketDataObject
{
	...
	public void Write(Packet packet)
	{
		...
		if (RequestId != 0)
		{
			...
			foreach (RequestRecord record in RequestRecords) record.Write(packet);
		}
		...
		foreach (TransactionRecord record in TransactionRecords) record.Write(packet);
	...
	{
		int requestId = packet.ReadSignedVarInt();
		var requestRecords = new List<RequestRecord>();

		if (requestId != 0)
		{
			int recordsCount = packet.ReadLength();
			for (int i = 0; i < recordsCount; i++) requestRecords.Add(RequestRecord.Read(packet));
		}
		...
		int count = packet.ReadLength();
		for (int i = 0; i < count; i++) records.Add(TransactionRecord.Read(packet));
...
public class RequestRecord : IPacketDataObject
{
	...
	public void Write(Packet packet)
	{
		...
		foreach (byte slot in Slots) packet.Write(slot);
	}

	public static RequestRecord Read(Packet packet)
	{
		var record = new RequestRecord { ContainerId = packet.ReadByte() };

		int slotsCount = packet.ReadLength();
...
public class ContainerTransactionRecord : TransactionRecord
{
	...
	{
		return new ContainerTransactionRecord { InventoryId = packet.ReadSignedVarInt() };
	}
}
...
public class WorldInteractionTransactionRecord : TransactionRecord
{
	...
	{
		return new WorldInteractionTransactionRecord { Flags = packet.ReadVarInt() };
	}
}
...
public class CreativeTransactionRecord : TransactionRecord
{
	...
	{
		return new CreativeTransactionRecord { InventoryId = 0x79 };
	}
}
...
public class CraftTransactionRecord : TransactionRecord
{
	...
	{
		return new CraftTransactionRecord { Action = (McpeInventoryTransaction.CraftingAction) packet.ReadVarInt() };
	}
}

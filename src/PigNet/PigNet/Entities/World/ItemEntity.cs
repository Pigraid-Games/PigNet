...
public class ItemEntity : Mob
{
	...
	{
		McpeAddItemActor mcpeAddItemEntity = McpeAddItemActor.CreateObject();
		mcpeAddItemEntity.entityIdSelf = EntityId;
		mcpeAddItemEntity.runtimeActorId = EntityId;
	...
	public override void OnTick(Entity[] entities)
	{
		...
		if (Velocity.Length() > 0.01)
		{
			...
			if (adjustAngle) CheckBlockAhead();
		...
		foreach (Player player in players)
			if (player.GameMode != GameMode.Spectator && bbox.Intersects(player.GetBoundingBox() + 1))
				if (player.PickUpItem(this))
				{
					...
					if (Item.Count > 0) Level.DropItem(KnownPosition, Item);
	...
	private Vector3 GetAdjustedLengthFromCollision(Vector3 velocity)
	{
		...
		{
			Vector3 distVec = direction * (float) length * i;
			BlockCoordinates blockPos = position + distVec;
			Block block = Level.GetBlock(blockPos);
			if (block.IsSolid)
			{
				var ray = new Ray(position, direction);
				double? distance = ray.Intersects(block.GetBoundingBox());
	...
	private void AdjustForCollision()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (block.IsSolid)
			{
				double yaw = (Math.Atan2(direction.X, direction.Z) * 180.0D / Math.PI) + 180;

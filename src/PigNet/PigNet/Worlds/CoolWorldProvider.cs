...
internal class SimplexOctaveGenerator
{
	...
	public SimplexOctaveGenerator(int seed, int octaves)
	{
		...
		for (int i = 0; i < _generators.Length; i++) _generators[i] = new SimplexPerlin(seed, NoiseQuality.Best);
	}
	...
	public double Noise(double x, double y, double z, double w, double frequency, double amplitude, bool normalized)
	{
		...
		foreach (SimplexPerlin octave in _generators)
		{
...
public class CoolWorldProvider : IWorldProvider
{
	...
	private readonly string _seed = Config.GetProperty("seed", "noise");
	...
	private void PopulateChunk(ChunkColumn chunk)
	{
		...
		for (int z = 0; z < 16; z++)
		{
			...
			for (int y = bottomHeight + 1; y > bottomHeight && y < maxHeight && y < 255; y++)
			{
				...
				if (!BlockFactory.IsBlock<Air>(thisblock) && BlockFactory.IsBlock<Air>(blockabove))
				{
			...
			for (int y = 0; y < 255; y++)
			{
				...
				{
					//Grass
					if (GetRandomNumber(0, 5) == 1) chunk.SetBlock(x, y + 1, z, new TallGrass());

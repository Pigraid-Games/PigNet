...
public class EnchantOptions : List<EnchantOption>, IPacketDataObject
	{
		public void Write(Packet packet)
		{
			...
			foreach (EnchantOption option in this) packet.Write(option);
		}
		...
		public static EnchantOptions Read(Packet packet)
		{
			...
			for (int i = 0; i < count; i++) options.Add(EnchantOption.Read(packet));
	...
	public class EnchantOption : IPacketDataObject
	{
		...
		public Enchants EquipActivatedEnchantments { get; set; } = [];
		public Enchants HeldActivatedEnchantments { get; set; } = [];
		public Enchants SelfActivatedEnchantments { get; set; } = [];
	...
	public class Enchants : List<Enchant>, IPacketDataObject
	{
		...
		{
			packet.WriteLength(Count);
			foreach (Enchant enchant in this) packet.Write(enchant);
		}
		...
		public static Enchants Read(Packet packet)
		{
			...
			int count = packet.ReadLength();
			for (int i = 0; i < count; i++) enchants.Add(Enchant.Read(packet));
			return enchants;
		}
	}
	...
	public class EnchantmentLevel(int minLevel, int maxLevel, byte level)
	{

...
public static class CryptoUtils
{
	...
	{
		byte[] keyType = { 0x45, 0x43, 0x4b, 0x33 };
		byte[] keyLength = { 0x30, 0x00, 0x00, 0x00 };
	...
	public static byte[] Encrypt(ReadOnlyMemory<byte> payload, CryptoContext cryptoContext)
	{
		...
		byte[] hashBufferPooled = ArrayPool<byte>.Shared.Rent(hashPoolLen);
		...
		byte[] encrypted = new byte[payload.Length + 8];
...
";
	...
	{
		var pubAsyKey = (ECPublicKeyParameters) key.Public;
		var privAsyKey = (ECPrivateKeyParameters) key.Private;
	...
	{
		if (input.Length == expectedSize) return input;
		...
		if (input.Length > expectedSize + 1 || input[0] != 0) throw new InvalidOperationException();

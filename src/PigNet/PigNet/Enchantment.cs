...
public static class Enchantment
{
	...
	{
		McpePlayerEnchantOptions enchantOptions = McpePlayerEnchantOptions.CreateObject();
	...
	public static void SendEnchantments(Player player, Item itemToEnchant)
	{
		...
		int bottomSlotLevel = Math.Max(baseLevel, noBookshelves * 2);
	...
	private static void AddBonusEnchants(Enchants enchants, Random rnd, int inLevel, ItemType itemType)
	{
		...
		do
		{
			Enchants possibleEnchantsForItem = GetPossibleEnchantsForItem(itemType, level);
	...
	{
		byte[] chars = new byte[rnd.Next(7, 20)];
		for (int i = 0; i < chars.Length; i++) chars[i] = (byte) rnd.Next('a', 'z');

...
using PigNet.Plugins.Commands;
using PigNet.Utils;
using PigNet.Worlds;

[assembly: InternalsVisibleTo("MiNETTests")]

namespace PigNet.Plugins;

public class PluginManager
{
	private static readonly ILog Log = LogManager.GetLogger(typeof(MiNetServer));
	private readonly Dictionary<MethodInfo, PacketHandlerAttribute> _packetHandlerDictionary = new();
	private readonly Dictionary<MethodInfo, PacketHandlerAttribute> _packetSendHandlerDictionary = new();
	private readonly Dictionary<MethodInfo, CommandAttribute> _pluginCommands = new();

	private string _currentPath;

	public List<object> Plugins { get; } = new();

	public CommandSet Commands { get; set; } = new();
					...
					try
					{
						...
						foreach (Type type in types)
							try
							{
								...
								if (Plugins.Any(l => l.GetType().AssemblyQualifiedName == type.AssemblyQualifiedName))
								{
	...
	{
		Assembly assembly = AssemblyResolve?.Invoke(sender, args);

		if (assembly != null) return assembly;
		if (_currentPath == null) return null;

		if (TryLoadAssembly(_currentPath ?? "", args.Name, out assembly)) return assembly;
	...
	{
		if (!Plugins.Contains(instance)) Plugins.Add(instance);
	...
	private void DebugPrintCommands()
	{
		...
		string content = JsonConvert.SerializeObject(Commands, settings);
	...
	private static string GetPropertyType(PropertyInfo parameter)
	{
		...
		if (parameter.PropertyType == typeof(int))
			...
			Log.Warn("No property type mapping for type: " + parameter.PropertyType);
	...
	{
		if (!Plugins.Contains(instance)) return;
		Plugins.Remove(instance);

		MethodInfo[] methods = _pluginCommands.Keys.Where(info => info.DeclaringType == instance.GetType()).ToArray();
		foreach (MethodInfo method in methods) _pluginCommands.Remove(method);
	...
	private void LoadPacketHandlers(Type type)
	{
		...
		foreach (MethodInfo method in methods)
		{
			var packetHandlerAttribute = Attribute.GetCustomAttribute(method, typeof(PacketHandlerAttribute), false) as PacketHandlerAttribute;
	...
	public void UnloadPacketHandlers(object instance)
	{
		...
		MethodInfo[] methods = _packetHandlerDictionary.Keys.Where(info => info.DeclaringType == instance.GetType()).ToArray();
		foreach (MethodInfo method in methods) _packetHandlerDictionary.Remove(method);
	}
	...
	internal void ExecuteStartup(MiNetServer server)
	{
		...
		{
			var startupClass = plugin as IStartup;
	...
	{
		foreach (object plugin in Plugins.ToArray())
		{
	...
	{
		string[] split = Regex.Split(cmdline, "(?<=^[^\"]*(?:\"[^\"]*\"[^\"]*)*) (?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)").Select(s => s.Trim('"')).ToArray();
		...
		foreach (Overload overload in command.Versions.First().Overloads.Values.OrderByDescending(o => o.Input.Parameters?.Length ?? 0))
		{
			...
			if (ExecuteCommand(method, player, args, out object retVal)) return retVal;
	...
	internal bool ExecuteCommand([NotNull] MethodInfo method, [NotNull] Player player, [NotNull] string[] args, out object result)
	{
		...
		try
		{
			...
			object pluginInstance = Plugins.FirstOrDefault(plugin => method.DeclaringType.IsInstanceOfType(plugin));
	...
	public Target FillTargets(Player commander, Level level, string source)
	{
		...
		else if (target.Selector == "closestPlayer" && target.Rules != null)
		{
			...
			IEnumerable<Player> players = level.GetAllPlayers().Where(p => p.Username == username);
	...
	public static Target ParseTarget(string source)
	{
		...
		{
			MatchCollection matches = Regex.Matches(source, @"^(?<selector>@[aeprs])(\[((?<args>(c|dx|dy|dz|l|lm|m|name|r|rm|rx|rxm|rym|type|x|y|z)=.*?)(,*?))*\])*$");
			string selector = matches[0].Groups["selector"].Captures[0].Value;
			switch (selector)
			{
				case "@a":
					selector = "allPlayers";
			...
			var rules = new List<Target.Rule>();
	...
	internal Packet PluginPacketHandler(Packet message, bool isReceiveHandler, Player player)
	{
		...
		try
		{
			...
			foreach (KeyValuePair<MethodInfo, PacketHandlerAttribute> handler in packetHandlers)
			{
				...
				{
					object pluginInstance = Plugins.FirstOrDefault(plugin => plugin.GetType() == method.DeclaringType);
					...
					if (method.ReturnType == typeof(void))
					{
						...
						else if (parameters.Length == 2 && typeof(Player).IsAssignableFrom(parameters[1].ParameterType)) method.Invoke(pluginInstance, new object[] { currentPacket, player });
					}
					else
					{
						ParameterInfo[] parameters = method.GetParameters();
						...
						else if (parameters.Length == 2 && typeof(Player).IsAssignableFrom(parameters[1].ParameterType)) returnPacket = method.Invoke(pluginInstance, new object[] { currentPacket, player }) as Packet;
					}
				}
			}
		}
	...
	public static string GetUsage(Command command, bool includeDescription = false, string prepend = "", string postpend = "")
	{
		...
		bool isFirst = true;
		foreach (Overload overload in command.Versions.First().Overloads.Values)
		{
			...
			if (overload.Input.Parameters != null)
				foreach (Parameter parameter in overload.Input.Parameters)
				{

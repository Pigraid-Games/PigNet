...
public interface ICommandExecutioner
	...
	public class PlaceAllBlocksExecutioner : ICommandExecutioner, IGenericPacketHandler
	{
		...
		private Item _lastItem = new ItemAir() { Count = 0 };
		...
		public void HandlePacket(BedrockTraceHandler caller, Packet packet)
		{
			switch (packet)
			{
				case McpeAddItemActor mcpePacket:
		...
		private void HandleMcpeInventoryContent(McpeInventoryContent message)
		{
			...
			{
				_inventory = message.slots;

				if (message.slots.Length > 0)
				{
					lock (_lastItem)
					{
						var item = message.slots.FirstOrDefault(x => x.Count > 0);
		...
		private void HandleMcpeAddItemEntity(McpeAddItemActor message)
		{

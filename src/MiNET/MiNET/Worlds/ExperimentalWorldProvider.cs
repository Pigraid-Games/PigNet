...
internal class ExperimentalWorldProvider : IWorldProvider
{
		...
		for (int z = 0; z < 16; z++)
		{
			...
			for (int y = 0; y < 256; y++)
			{
				...
				if (y <= stoneHeight)
				{
					...
					//Diamond ore
					if (GetRandomNumber(0, 2500) < 5) chunk.SetBlock(x, y, z, new DiamondOre());

					//Coal Ore
					if (GetRandomNumber(0, 1500) < 50) chunk.SetBlock(x, y, z, new CoalOre());

					//Iron Ore
					if (GetRandomNumber(0, 2500) < 30) chunk.SetBlock(x, y, z, new IronOre());

					//Gold Ore
					if (GetRandomNumber(0, 2500) < 20) chunk.SetBlock(x, y, z, new GoldOre());
				}
				...
				if (y <= dirtHeight && y >= stoneHeight)
				{
					...
					{
						//Grass
						if (GetRandomNumber(0, 5) == 2) chunk.SetBlock(x, y + 2, z, new Tallgrass() { TallGrassType = "tall" });
				...
				if (y == 0) chunk.SetBlock(x, y, z, new Bedrock());
			}
		}
	}
	...
	private static readonly OpenSimplexNoise OpenNoise = new("a-seed".GetHashCode());

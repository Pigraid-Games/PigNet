...
public class SubChunk : IDisposable, ICloneable
{
	...
	private List<int> _loggedRuntimeIds = new();
	...
	{
		if (IsDirty) _isAllAir = AllZeroFast(_blocks);
		return _isAllAir;
	}
	...
	{
		int paletteIndex = _runtimeIds.IndexOf(runtimeId);
		if (paletteIndex == -1)
		{
	...
	{
		int paletteIndex = _loggedRuntimeIds.IndexOf(runtimeId);
		if (paletteIndex == -1)
		{
	...
	public void Write(MemoryStream stream)
	{
		...
		List<int> runtimeIds = _runtimeIds;
		short[] blocks = _blocks;

		if (runtimeIds != null && runtimeIds.Count > 0)
			numberOfStores++;

		List<int> loggedRuntimeIds = _loggedRuntimeIds;
		byte[] loggedBlocks = _loggedBlocks;

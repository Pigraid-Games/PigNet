...
namespace MiNET.Utils.Collections;
	...
	where TKey : IComparable<TKey>
{
	...
	private readonly MinBinaryHeap _minHeap = new();
	...
	public ConcurrentPriorityQueue(IEnumerable<KeyValuePair<TKey, TValue>> collection)
	{
		...
		foreach (KeyValuePair<TKey, TValue> item in collection) _minHeap.Insert(item);
	}
	...
	public KeyValuePair<TKey, TValue>[] ToArray()
	{
		lock (_syncLock)
		{
			...
			for (int i = 0; i < result.Length; i++) result[i] = clonedHeap.Remove();
			return result;
		}
	}
	...
	private sealed class MinBinaryHeap
	{
		...
		public void Insert(KeyValuePair<TKey, TValue> entry)
		{
			...
			while (pos > 0)
			{
				...
				KeyValuePair<TKey, TValue> toCheck = _items[nextPos];
			...
			else
			{
				...
				while (true)
				{
					...
					if (leftChildPos < _items.Count)
					{
						...
						KeyValuePair<TKey, TValue> entry1 = _items[current];
						KeyValuePair<TKey, TValue> entry2 = _items[leftChildPos];
					...
					{
						// Get the two entries to compare (node and its left child)
						KeyValuePair<TKey, TValue> entry1 = _items[possibleSwap];
						KeyValuePair<TKey, TValue> entry2 = _items[rightChildPos];
					...
					{
						KeyValuePair<TKey, TValue> temp = _items[current];

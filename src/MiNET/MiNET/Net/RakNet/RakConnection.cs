...
public class RakConnection : IPacketSender
{
	...
	public void Stop()
	{
		try
		{
			...
			foreach (KeyValuePair<IPEndPoint, RakSession> rakSession in _rakSessions) rakSession.Value.Close();
	...
	private static UdpClient CreateListener(IPEndPoint endpoint)
	{
		...
		if (Environment.OSVersion.Platform != PlatformID.Unix && Environment.OSVersion.Platform != PlatformID.MacOSX)
		{
			...
			listener.Client.IOControl((int) SIO_UDP_CONNRESET, new[] { Convert.ToByte(false) }, null);
	...
	private Packet HandleSplitMessage(RakSession session, SplitPartPacket splitPart)
	{
		...
		foreach (SplitPartPacket spp in splitPartList) contiguousLength += spp.Message.Length;
	...
	private async void SendTick(object obj)
	{
		...
		foreach (KeyValuePair<IPEndPoint, RakSession> session in _rakSessions) tasks.Add(session.Value.SendTickAsync(this));

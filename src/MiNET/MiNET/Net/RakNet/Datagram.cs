...
public class Datagram : Packet<Datagram>
{
	...
	// From decoding
	public List<Packet> Messages { get; set; } = new();
	...
	public override void Reset()
	{
		...
		foreach (MessagePart part in MessageParts) part.PutPool();
	...
	{
		List<(int from, int length)> result = new();

		int i = 0;
		for (; (i + 1) * intBufferLength < length; i++) result.Add((i * intBufferLength, intBufferLength));

		(int form, int length) reminder = (i * intBufferLength, length - (i * intBufferLength));
		if (reminder.length > 0) result.Add(reminder);

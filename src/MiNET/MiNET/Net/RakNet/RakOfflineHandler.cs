...
public sealed class RakOfflineHandler
{
	...
	private void HandleRakNetMessage(IPEndPoint senderEndpoint, UnconnectedPing message)
	{
...

		{
			...
			UnconnectedPong packet = UnconnectedPong.CreateObject();
	...
	public void HandleRakNetMessage(IPEndPoint senderEndpoint, UnconnectedPong message)
	{
		...
		if (!HaveServer)
		{
			...
			if (motdParts.Length >= 11) senderEndpoint.Port = int.Parse(motdParts[10]);
	...
	public void SendOpenConnectionRequest1(IPEndPoint targetEndPoint, short mtuSize)
	{
		...
		OpenConnectionRequest1 packet = OpenConnectionRequest1.CreateObject();
	...
	private void HandleRakNetMessage(IPEndPoint senderEndpoint, OpenConnectionRequest1 message)
	{
		...
		OpenConnectionReply1 packet = OpenConnectionReply1.CreateObject();
	...
	{
		if (message.MtuSize != MtuSize) Log.Debug($"Error, mtu differ from what we sent. Received {message.MtuSize} bytes");
		//return;
	...
	{
		OpenConnectionRequest2 packet = OpenConnectionRequest2.CreateObject();
	...
	private void HandleRakNetMessage(IPEndPoint senderEndpoint, OpenConnectionRequest2 incoming)
	{
		...
		OpenConnectionReply2 reply = OpenConnectionReply2.CreateObject();
	...
	private void SendConnectionRequest(IPEndPoint targetEndPoint, short mtuSize)
	{
		...
		ConnectionRequest packet = ConnectionRequest.CreateObject();
		packet.clientGuid = ClientGuid;
	...
	internal static void TraceReceive(ILog log, Packet message)
	{
		...
		try
		{
			...
			if (!Regex.IsMatch(typeName, includePattern)) return;

			if (!string.IsNullOrWhiteSpace(excludePattern) && Regex.IsMatch(typeName, excludePattern)) return;
			...
			else if (verbosity == 2 || verbosity == 3) log.Verbose($"> Receive: {message.Id} (0x{message.Id:x2}): {message.GetType().Name}\n{Packet.HexDump(message.Bytes)}");
		}
		catch (Exception e)
		{
	...
	internal static void TraceSend(Packet message)
	{
		...
		try
		{
			...
			else if (verbosity == 2 || verbosity == 3) Log.Verbose($"<    Send: {message.Id} (0x{message.Id:x2}): {message.GetType().Name}\n{Packet.HexDump(message.Bytes)}");
		}
		catch (Exception e)
		{
			Log.Error("Error when printing trace", e);

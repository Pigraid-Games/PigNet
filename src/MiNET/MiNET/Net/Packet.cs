...
//SOCKADDR_IN6_LH, * PSOCKADDR_IN6_LH, FAR * LPSOCKADDR_IN6_LH;
	...
	public void Write(CreativeItemStacks itemStacks)
	{
		...
		int netId = 0;
		foreach (Item item in itemStacks)
		{
	...
	public void Write(ItemComponentList list)
	{
		...
		foreach (ItemComponent item in list)
		{
			Write(item.Name);
			Write(item.Nbt);
		}
	...
	private const byte FurnaceAux = 3;
	private const byte Multi = 4;
	private const byte UserDataShapeless = 5;
	...
	public HeightMapData ReadHeightMapData()
	{
		...
		for (int i = 0; i < heights.Length; i++) heights[i] = (short) ReadByte();
	...
	private object _encodeSync = new();

	private static RecyclableMemoryStreamManager _streamManager = new();
	private static ConcurrentDictionary<int, bool> _isLob = new();
...
public abstract partial class Packet<T> : Packet, IDisposable where T : Packet<T>, new()
{
	...
	[JsonIgnore] public bool IsPooled => _isPooled;

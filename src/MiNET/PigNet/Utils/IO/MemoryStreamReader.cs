...
public class MemoryStreamReader : Stream
{
	...
	{
		short val = BinaryPrimitives.ReadInt16LittleEndian(_buffer.Span.Slice((int) Position, 2));
		Position += 2;
		return val;
	}
	...
	{
		ushort val = BinaryPrimitives.ReadUInt16LittleEndian(_buffer.Span.Slice((int) Position, 2));
		Position += 2;
		return val;
	}
	...
	{
		ulong length = ReadVarLongInternal();
		return Read(length);
	}

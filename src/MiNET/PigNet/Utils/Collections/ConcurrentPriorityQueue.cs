...
namespace PigNet.Utils.Collections;
	...
	private sealed class MinBinaryHeap
	{
		...
		public void Insert(KeyValuePair<TKey, TValue> entry)
		{
			...
			while (pos > 0)
			{
				...
				KeyValuePair<TKey, TValue> toCheck = Items[nextPos];
			...
			else
			{
				...
				while (true)
				{
					...
					if (leftChildPos < Items.Count)
					{
						...
						KeyValuePair<TKey, TValue> entry1 = Items[current];
						KeyValuePair<TKey, TValue> entry2 = Items[leftChildPos];
					...
					{
						// Get the two entries to compare (node and its left child)
						KeyValuePair<TKey, TValue> entry1 = Items[possibleSwap];
						KeyValuePair<TKey, TValue> entry2 = Items[rightChildPos];
					...
					{
						KeyValuePair<TKey, TValue> temp = Items[current];
